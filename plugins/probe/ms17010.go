...
func Ms17010Probe(task model.ProbeTask) (result model.ProbeTaskResult) {
	result = model.ProbeTaskResult{ProbeTask: task, Result: "", Err: nil}
	// connecting to a host in LAN if reachable should be very quick
	conn, err := net.DialTimeout("tcp", task.Ip+":445", model.ConnectTimeout)
	...
	if reply[9] == 0x05 && reply[10] == 0x02 && reply[11] == 0x00 && reply[12] == 0xc0 {
		...
		if reply[34] == 0x51 {
			...
			r := fmt.Sprintf("[+] %s has DOUBLEPULSAR SMB IMPLANT", task.Ip)
			result.Result = r
		}

	} else {
		r := fmt.Sprintf("%s  (%s)", task.Ip, os)
		result.Result = r
	}
	return result
