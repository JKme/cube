//https://raw.githubusercontent.com/paulstuart/ping/master/ping.go
package probe
...
func PingProbe(task model.ProbeTask) (result model.ProbeTaskResult) {
...
func Ping(address string, timeout int) (r bool) {
	//err := Pinger(address, timeout)
	c, err := net.Dial("ip4:icmp", address)
	if err != nil {
		r = execCommandPing(address)
	} else {
		err = Pinger(c, timeout)
		if err == nil {
			r = true
		}
	}
...
func Pinger(c net.Conn, timeout int) error {
	//c, err := net.Dial("ip4:icmp", address)
	//if err != nil {
	//	//execCommandPing(address)
	//
	//	//return err
	//}
