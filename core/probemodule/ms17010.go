...
func (m Ms17010) ProbeName() string {
	return "ms17010"
}

func (m Ms17010) ProbePort() string {
	return "445"
}
...
func (m Ms17010) ProbeExec() ProbeResult {
	...
	if reply[9] == 0x05 && reply[10] == 0x02 && reply[11] == 0x00 && reply[12] == 0xc0 {
		...
		r := fmt.Sprintf("[+] %s\tMS17-010 Vulnerable\t(%s)", m.Ip, os)
		result.Result = r
		...
		if reply[34] == 0x51 {
			...
			r := fmt.Sprintf("[+] %s has DOUBLEPULSAR SMB IMPLANT", m.Ip)
			result.Result = r
		}

	} else {
		r := fmt.Sprintf("%s  (%s)", m.Ip, os)
		result.Result = r
	}
	return result
}
