...
type SmbGhost struct {
	*Probe
}

func (s SmbGhost) ProbeName() string {
	return "smbghost"
}

func (s SmbGhost) ProbePort() string {
	return "445"
}
...
func (s SmbGhost) ProbeExec() ProbeResult {
	...
	addr := fmt.Sprintf("%s:%v", s.Ip, s.Port)
	conn, err := net.DialTimeout("tcp", addr, config.TcpConnTimeout)
	if err != nil {
		return result
	}
	...
	if bytes.Contains(buff[:n], []byte("Public")) == true {
		result.Result = fmt.Sprintf("%v CVE-2020-0796 SmbGhost Vulnerable", s.Ip)
